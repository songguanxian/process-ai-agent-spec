...
人工兜底交互必须出现在 AI 会话卡片、OCR 确认页、表单草稿卡和固定确认页中。系统不得只展示错误文本后中断流程。
...
| 人工修正/补录 | 授权项目 | 授权项目 | 否 | 授权项目 | 否 | 否 | 否 |
| 已确认批次改绑/作废 | 否 | 授权项目 | 否 | 授权项目 | 否 | 按授权 | 否 |
| 复核样本池查看 | 否 | 视权限 | 视权限 | 视权限 | 是 | 是 | 技术问题样本 |
...
| 创建/完成人工兜底 | 记录低置信度、无结果、多候选、识别错误或工具失败后的人工处理过程和最终结果。 |
